...
ERROR_DATA = f"{colors.RED}> wrong data:{colors.RESET} "
...
def clear_date(_date) -> str:
    """Перевірка, чи введена дата коректна та чи не більша за сьогоднішню"""

    n_date = _normalize_date(_date)
    # n_date = _switch_to_dmy(_date_to_str(_date).replace("-", ".").replace("/", "."))
...
if __name__ == "__main__":
    ...
    # print(list_dates(clear_date(date.today()), 3))
    # print(list_dates(clear_date("2023-11/30"), 5))
